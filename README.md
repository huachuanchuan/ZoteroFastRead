...
1. 打开zotero的【工具】-【插件】，然后导入xpi文件（加载有点慢请耐心等待）
![图片 2](readme-assets/image2.jpeg)

![图片 3](readme-assets/image3.jpeg)

1. 在zotero的【编辑】-【设置】-【fastRead设置】配置API key（默认为deepseek），然后点击测试连接，等待结果，如果为绿色则表示模型以接通，可以选择启用。
...
1. 由于各种现在，文献翻译速率较慢，需要耐心等待（平均5min左右翻译一篇）。建议使用者在第一天晚上开始翻译，第二天即可全部翻译完成。对于急于看文献的使用者不建议使用本插件。
1. 由于插件由作者vibe coding完成，所以作者未发现的问题恳请使用者反馈。
1. 作者目前大模型只测试了deepseek和qwen，使用者接入其他模型的时候有错误可以及时向作者反馈。
谢谢大家！
